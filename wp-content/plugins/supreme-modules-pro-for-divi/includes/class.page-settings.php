	...
	class DSM_Settings {
		...
		function admin_notices() {
			...
			if ( $messages && count( $messages ) > 0 ) {
				echo "<div class='notice-warning notice fade'><p>" . implode( ',', $messages ) . '</p></div>';
			}
		}
		...
		function licence_form_submit() {
			...
			if ( isset( $_POST['dsm_pro_license_form_submit'] ) && isset( $_POST['dsm_pro_licence_deactivate'] ) && isset( $_POST['dsm_pro_license'] ) && wp_verify_nonce( sanitize_key( $_POST['dsm_pro_license'] ), 'dsm_pro_license' ) ) {
				...
				if ( isset( $response_block->status ) ) {
					if ( 'success' === $response_block->status && 's201' === $response_block->status_code ) {
						...
						if ( 'e002' === $response_block->status_code || 'e104' === $response_block->status_code ) {
							$license_data = get_site_option( 'dsm_pro_license' );
					...
					$slt_form_submit_messages[] = __( 'There was a problem with the data block received from ', 'dsm-supreme-modules-pro-for-divi' ) . DSM_PRO_APP_API_URL;
					return;
				}
			...
			if ( isset( $_POST['dsm_pro_license_form_submit'] ) && wp_verify_nonce( sanitize_key( $_POST['dsm_pro_license'] ), 'dsm_pro_license' ) ) {
				...
				if ( isset( $response_block->status ) ) {
					if ( 'success' === $response_block->status && 's100' === $response_block->status_code ) {
							// the license is active and the software is active
							$slt_form_submit_messages[] = $response_block->message;
							...
							<tr>
								...
								<td><?php if ( $this->licence->is_local_instance() ) { ?>
									...
									<?php
										...
										?>
									<b><?php echo substr( $licese_data_key, 0, 20 ); ?>-xxxxxxxx-xxxxxxxx</b>
									<?php } ?></td>
							</tr>
						</tbody>
					</table>
