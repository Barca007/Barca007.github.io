...
/**
 * Require templates customizer
 * 
 * @since 4.7.5.2
 */
require_once( TEMPLATEPATH . '/regsite/templates_customize.php' );
...
$plugins_name = array_slice( scandir( $plugins_path ), 2 );
foreach( $plugins_name as $plugin ) {
...
function et_theme_epanel_reminder(){
	...
	$documentation_url         = 'https://www.reg.ru/support/sozdanie-sajtov/reg-site/#expanders=c0,c1';
 ...
 * @since 4.6.5.1
 */
require 'core/components/Updates.php';
$update_checker = new DiviUpdater();


/**
 * Add Divi Import dashboard widget.
 */
function di_dashboard_widget() {
	echo "Воспользуйтесь возможностью установить готовый сайт.";
	echo '<p><a href="' . home_url() . '/wp-admin/themes.php?page=divi-import"><button class="button button-primary">К шаблонам</button></a></p>';
}
function di_add_dashboard_widget() {
	wp_add_dashboard_widget( 'di_add_dashboard_widget', 'Выбрать шаблон для сайта', 'di_dashboard_widget' );
}
add_action( 'wp_dashboard_setup', 'di_add_dashboard_widget' );


/**
 * Remove widgets
 */
function regsite_remove_all_dashboard_metaboxes() {
    // Remove Welcome panel
    remove_action( 'welcome_panel', 'wp_welcome_panel' );
    // Remove the rest of the dashboard widgets
    remove_meta_box( 'dashboard_primary', 'dashboard', 'side' );
}
add_action( 'wp_dashboard_setup', 'regsite_remove_all_dashboard_metaboxes' );
...
function enqueue_custom_scripts() {
	...
	wp_enqueue_style( 'custom-styles', get_template_directory_uri() . '/style-custom.css', array(), $theme_info->get( 'Version' ) );

	if( current_user_can( 'publish_posts' ) ) {
		wp_enqueue_script( 'custom-scripts', get_template_directory_uri() . '/custom.main.js', array(), $theme_info->get( 'Version' ), false );
	}
}
...
/**
 * Add regru-lk-link button to admin bar menu
 */
if ( current_user_can('administrator') ) {
	add_action( 'admin_bar_menu', function ( $wp_admin_bar ) {
		$wp_admin_bar->add_menu( array(
			'id'     => 'regru-lk-link',
			'title'  => '<img style="width: 40px; position: relative; top:5px; left: -2px;" src="'.get_template_directory_uri().'/images/logo_regru.svg" /> <span>Личный кабинет</span>',
			'href'   => 'https://www.reg.ru/user/account/',
			'meta'  => array(
				'target'=> '_blank'
			)
		) );
...
add_action( 'wp_head', function () {
	?>
	<script type="text/javascript">
		var is_admin = <?php echo json_encode( current_user_can('administrator') ); ?>;
	</script>
	<?php
}, 1 );
...
/**
 * Add node to settings admin bar menu
 */
add_action( 'admin_bar_menu', 'settings_links', 50 );
function settings_links($wp_admin_bar)
{
	$args = array(
		'id'     => 'settings-link',
		'title'	=>	'<span class="ab-icon"></span> Настройки сайта',
	...
	array_push($args,array(
		...
		'href'		=>	get_admin_url( null, 'admin.php?page=et_divi_options' ),
		'parent'	=>	'settings-link',
	));
	array_push($args,array(
		'id'		=>	'other-settings',
		'title'		=>	'Дополнительные настройки',
		'href'		=>	get_admin_url( null, 'customize.php' ),
		'parent'	=>	'settings-link',
	));
	array_push($args,array(
		'id'     	=> 'navigation-settings',
		'title'		=>	'Навигация',
		'href'		=>	get_admin_url( null, 'admin.php?page=et_divi_options#wrap-navigation' ),
		'parent' 	=> 'settings-link',
		'meta'   	=> array( 'class' => 'first-toolbar-group' ),
	));
	array_push($args,array(
		'id'		=>	'seo-settings',
		'title'		=>	'SEO настройки',
		'href'		=>	get_admin_url( null, 'admin.php?page=et_divi_options#wrap-seo' ),
		'parent'	=>	'settings-link',
	));
...
/**
 * Remove unnecessary items from admin bar menu,
 * Changes for admin bar site-name node
 */
function admin_bar_edit_items() {
	global $wp_admin_bar;
	...
	//Rename Dashboard to Adminka
	if ( $wp_admin_bar->get_node('dashboard') ) {
		$newDashboardNode = $wp_admin_bar->get_node('dashboard');
		$newDashboardNode->title = esc_html__( 'Adminka', 'Divi' );
		if ( $newDashboardNode->parent ) $wp_admin_bar->add_node($newDashboardNode);
	}

	//Rename Themes to Choose Template
	if ( $wp_admin_bar->get_node('themes') ) {
		$newThemesNode = $wp_admin_bar->get_node('themes');
		$newThemesNode->title = esc_html__( 'Choose Template', 'Divi' );
		$newThemesNode->href = get_admin_url( null, 'themes.php?page=divi-import' );
		if ( $newThemesNode->parent ) $wp_admin_bar->add_node($newThemesNode);
	}
}
add_action('wp_before_admin_bar_render','admin_bar_edit_items', 0 );


/**
 * Remove et_automatic_updates_options
 */
if ( get_option( 'et_automatic_updates_options' ) ) {
	delete_option( 'et_automatic_updates_options' );
}

/**
 * Add help button to admin bar menu
 */
add_action( 'admin_bar_menu', function ( $wp_admin_bar ) {
	$wp_admin_bar->add_menu( array(
		'id'     => 'video',
		'parent' => 'top-secondary',
		'title'  => '<span class="ab-icon"></span> <span class="ab-title">Первый взгляд</span>',
		'meta'  => array(
			'class' => 'admin_bar_menu__video'
		)
	) );
}, 1000 );
...
add_action( 'admin_bar_menu', function ( $wp_admin_bar ) {
	$wp_admin_bar->add_menu( array(
		'id'     => 'support-link',
		'parent' => 'top-secondary',
		'title'  => '<span class="ab-icon"></span> Помощь',
		'href'   => 'https://www.reg.ru/support/sozdanie-sajtov/reg-site/#expanders=c0,c1',
		'meta'  => array(
			'target'=> '_blank'
		)
	) );
...
/**
 * Saves relative URLs media to database when save 
 * 
 * @since 4.7.5.1
 * @param string $content
 * @return string
...
/**
 * Allow viber protocol in HTML attributes.
 * 
 * @since 4.7.5.1
